...
/*   Updated: 2021/12/07 19:29:54 by signacia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_textures(t_all *data, char *str)
{
		...
		|| ft_strncmp(str, "WE ", 3) == 0 || ft_strncmp(str, "EA ", 3) == 0)
	{
		if (ft_strncmp(str, "NO ", 3) == 0)
			...
			x = 3;
		data->texture_addr[x] = ft_strtrim(str + 3, " \t");
		if (data->texture_addr[x] == NULL
			|| ft_strlen(data->texture_addr[x]) == 0)
	...
	if (ft_strncmp(str, "F ", 2) == 0 || ft_strncmp(str, "C ", 2) == 0)
	{
		if (ft_strncmp(str, "F ", 2) == 0)
		{
			if (data->floor_color != -1)
				common_err(ERR_ARGUMENTS);
			data->floor_color = color_converting(ft_strtrim(str + 2, " \t"));
		}
		else
		{
			if (data->ceil_color != -1)
				common_err(ERR_ARGUMENTS);
			data->ceil_color = color_converting(ft_strtrim(str + 2, " \t"));
		}
		return (1);
	}
	return (0);
...
void	parse_map(t_all *data, char *line)
{
	...
	if (data->map_arr != NULL)
	{
		...
		if (!tmp[i])
			common_err(ERR_MALLOC);
		while (i-- > 0)
			tmp[i] = data->map_arr[i];
		free(data->map_arr);
		data->map_arr = tmp;
		return ;
	}
	data->map_arr = (char **)ft_calloc(2, sizeof(char *));
	data->map_arr[0] = ft_strdup(line);
	data->map_arr[1] = NULL;
}
...
void	parsing_qube(char *file_name, t_all *data)
{
	...
	data->map_arr = NULL;
	data->texture_addr = (char **)ft_calloc(sizeof(char *), 4);
	data->floor_color = -1;
	data->ceil_color = -1;
	while (1)
	{
